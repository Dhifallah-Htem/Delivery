(function() {
	...
	.controller(
			'MenuController',
			[ '$scope', '$attrs', '$location', '$rootScope', 'menuConfig',
					MenuController ])
	...
	// The ml-menu-group directive indicates a block of html that will expand
	// and collapse in a menu
	.directive('mlMenuGroup', MenuGroup)

	// Use ml-menu-heading below an menu-group to provide a heading containing
	// HTML
	// <ml-menu-group>
	// <ml-menu-heading>Heading containing HTML - <img
	// src="..."></ml-menu-heading>
	// </ml-menu-group>
	.directive('mlMenuGroupHeading', MenuGroupHeading)

	// Use in the menu-group template to indicate where you want the heading to
	// be transcluded
	// You must provide the property on the ml-menu-group controller that will
	// hold the transcluded element
	// <li>
	// <a class="mdl-navigation__link" ><span ...
	// ml-menu-transclude="heading">...</span></a>
	// ...
	// </li>
	...
	function MenuController($scope, $attrs, $location, $rootScope, menuConfig) {
		...
		vm.closeOthers = function(openGroup) {
			var closeOthers = angular.isDefined($attrs.closeOthers) ? $scope
					.$eval($attrs.closeOthers) : menuConfig.closeOthers;
			if (closeOthers) {
...
//		};
		...
		// This is called from the ml-menu-group directive to add itself to the
		// menu
...
//		};
	...
	function MenuItem() {
		return {
			require : '^mlMenu', // We need this directive to be inside a
			// menu
			restrict : 'EA',
			transclude : true, // It transcludes the contents of the directive
			// into the template
...
//			};
	...
	function MenuGroup() {
		return {
			...
			scope : {
				heading : '@', // Interpolate the heading attribute onto this
				// scope
				path : '@',
				isOpen : '=?'
			},
	...
	function collapse($animate) {
		...
		function link(scope, element, attrs) {
			...
			function collapseDone() {
				element.css({
					height : '0'
				}); // Required so that collapse works when animation is
				// disabled
				element.removeClass('collapsing');
